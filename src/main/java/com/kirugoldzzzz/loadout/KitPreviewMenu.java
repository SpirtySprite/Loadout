...
public final class KitPreviewMenu {
    ...
    public void open(Player player, Kit kit, Runnable back) {
        ...
        Gui gui = Gui.builder()
                .rows(ROWS)
                .title(Mini.parse(Palette.smallTitle(Tr.t("Aperçu du kit"))))
                .create();
        Guis.fill(gui);
    ...
    private GuiItem slot(Kit kit, int slot) {
        ...
        if (item == null) {
            return Guis.display(KitSlots.equipment(slot) ? Material.GRAY_STAINED_GLASS_PANE
                    ...
                    Palette.MUTED + KitSlots.label(slot), List.of(Palette.MUTED + Tr.t("Vide")));
        }
        ...
        if (meta != null) {
            ...
            if (KitSlots.equipment(slot) && kit.options().autoEquip()) {
                lore.add(Mini.label(Card.noteLine(Palette.SUCCESS, Palette.CHECK, Tr.t("Équipé automatiquement si libre"))));
            }
            if (kit.options().protectItems()) {
                lore.add(Mini.label(Card.noteLine(Palette.WARNING, "⛓", Tr.t("Invendable"))));
            }
            meta.lore(lore);
    ...
    private GuiItem rewards(Kit kit) {
        Card card = Card.of(KitStyle.HEX).tag(Tr.t("Kit"));
    ...
    private GuiItem poolButton(Kit kit, Runnable back) {
        ...
        Card card = Card.of(KitStyle.HEX).tag(Tr.t("Tirage aléatoire")).blank()
                .stat(Card.CHANCE, Tr.t("Tirages"), pool.effectiveRolls() + (pool.unique() ? Tr.t(" sans doublon") : Tr.t(" avec doublons possibles")))
                .count(Card.AMOUNT, Tr.t("Possibilités"), pool.entries().size())
                .blank()
                .click(Tr.t("pour voir les chances"));
        return new GuiItem(KitStyle.decorate(new ItemStack(Material.ENDER_EYE), Palette.heading(Tr.t("Tirage aléatoire")),
    ...
    public void openPool(Player player, Kit kit, Runnable back) {
        Gui gui = Gui.builder()
                .rows(ROWS)
                .title(Mini.parse(Palette.smallTitle(Tr.t("Tirage du kit"))))
                .create();
        Guis.fill(gui);
        KitPool pool = kit.pool();
        ...
        gui.setItem(ROWS, 5, Guis.display(Material.ENDER_EYE, Palette.heading(Tr.t("Comment ça marche")),
                Card.of(KitStyle.HEX).blank()
                        .line(pool.effectiveRolls() + Tr.t(" objet(s) tiré(s) à chaque récupération."))
                        .line(pool.unique() ? Tr.t("Un même objet ne sort qu'une fois.") : Tr.t("Un objet peut sortir plusieurs fois."))
                        .line(pool.exact() ? Tr.t("Les pourcentages sont exacts.") : Tr.t("Part de chaque objet dans un tirage."))
                        .build()));
        if (back != null) {
            gui.setItem(ROWS, Guis.BACK_SLOT, Guis.backButton(back));
        }
    ...
    private GuiItem poolEntry(KitPool pool, KitPool.Entry entry) {
        ...
        if (meta != null) {
            List<Component> lore = meta.lore() == null ? new ArrayList<>() : new ArrayList<>(meta.lore());
            lore.add(Component.empty());
            String label = pool.exact() ? Tr.t("Chance d'obtention") : Tr.t("Part du tirage");
            ...
            lore.add(Mini.label(Card.noteLine(Palette.SECONDARY, Card.AMOUNT, Tr.t("Quantité : ") + Palette.SECONDARY + amount)));
            meta.lore(lore);
    ...
    private GuiItem claimButton(Gui gui, Player player, Kit kit, KitStatus status, Runnable back) {
        Card card = Card.of(KitStyle.HEX).tag(Tr.t("Récupération")).blank().raw(KitStyle.stateLine(status,
        ...
        if (discount > 0) {
            card.raw(Palette.WARNING + Tr.t("⚡ Kit du jour ") + Palette.SUCCESS + "-" + discount + "%");
        }
        if (status.available()) {
            card.click(kit.cost().free() ? Tr.t("pour récupérer le kit") : Tr.t("pour acheter et récupérer"));
        } else {
            card.deny(Tr.t("Pas encore disponible"));
        }
        ...
        return new GuiItem(KitStyle.decorate(new ItemStack(icon),
                (status.available() ? Palette.SUCCESS : Palette.ERROR) + "<b>" + Card.small(Tr.t("Récupérer")) + "</b>",
    ...
    private GuiItem tryOnButton(Kit kit) {
        List<String> lore = Card.of(KitStyle.HEX).tag(Tr.t("Essayage")).blank()
                .line(Tr.t("Un mannequin holographique porte"))
                .line(Tr.t("l'armure du kit devant vous"))
                .line(Tr.t("pendant 10 secondes."))
                .blank()
                .click(Tr.t("pour essayer"))
                .build();
        return new GuiItem(KitStyle.decorate(new ItemStack(Material.ARMOR_STAND), Palette.heading(Tr.t("Essayer l'armure")),
                lore, false), event -> {
            Player viewer = (Player) event.getWhoClicked();
            Guis.click(viewer);
    ...
    private GuiItem masteryButton(Player player, Kit kit, Runnable back) {
        ...
        Card card = Card.of(KitStyle.HEX).tag(Tr.t("Maîtrise")).blank()
                .stat(Card.STAR, Tr.t("Palier"), tier == null ? Tr.t("aucun") : tier.name())
        ...
        card.blank().click(Tr.t("pour voir les paliers"));
        return new GuiItem(KitStyle.decorate(new ItemStack(Material.EXPERIENCE_BOTTLE), Palette.heading(Tr.t("Maîtrise")),
    ...
    private GuiItem giftButton(Kit kit, Runnable back) {
        List<String> lore = Card.of(KitStyle.HEX).tag(Tr.t("Cadeau")).blank()
                .line(Tr.t("Payez ce kit pour un autre joueur,"))
                .line(Tr.t("il reçoit un bon à ouvrir."))
                .blank()
                .click(Tr.t("pour choisir le joueur"))
                .build();
        return new GuiItem(KitStyle.decorate(new ItemStack(Material.CAKE), Palette.heading(Tr.t("Offrir ce kit")), lore,
                false), event -> {
            Player viewer = (Player) event.getWhoClicked();
            Guis.click(viewer);
